...
using GroupDocsTextVisualStudioPlugin.Core;
using System.IO;
using System.Threading;
using System.Xml;
using GroupDocsTextVisualStudioPlugin.XML;
using System.Xml.Serialization;
using System.Diagnostics;
using System.Xml.Linq;
using EnvDTE80;

namespace GroupDocsTextVisualStudioPlugin.GUI
{
    public partial class SampleWizardPage : Form
    {
        ...
        public SampleWizardPage(DTE2 application)
        {
            ...
            PopulateTreeView(repoPath + "/Examples/" + (rdbCSharp.Checked ? "CSharp\\GroupDocs.Text.Examples.CSharp" : "VBasic\\GroupDocs.Text.Examples.VBasic"));
        ...
        bool CopyAndCreateProject()
        {
            ...
            //string sampleSourcePath = "GroupDocs.Text";
            ...
            try
            {
                ...
                {
                    //For CSharp Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "CSharp\\GroupDocs.Text.Examples.CSharp", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "CSharp\\GroupDocs.Text.Examples.CSharp", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "CSharp\\GroupDocs.Text.Examples.CSharp", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);

                    //For VBasic Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "VBasic\\GroupDocs.Text.Examples.VBasic", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "VBasic\\GroupDocs.Text.Examples.VBasic", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "VBasic\\GroupDocs.Text.Examples.VBasic", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);
                }

                progressBar1.Value = 50;

                string[] projectFiles = Directory.GetFiles(Path.Combine(destinationPath, (rdbCSharp.Checked ? "CSharp\\GroupDocs.Text.Examples.CSharp" : "VBasic\\GroupDocs.Text.Examples.VBasic")), (rdbCSharp.Checked ? "*.csproj" : "*.vbproj"));
