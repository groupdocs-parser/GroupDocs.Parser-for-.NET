...
using GroupDocsParserVisualStudioPlugin.Core;
using System.IO;
...
namespace GroupDocsParserVisualStudioPlugin.GUI
{
    public partial class SampleWizardPage : Form
    {
        ...
        public SampleWizardPage(DTE2 application)
        {
            ...
            PopulateTreeView(repoPath + "/Examples/" + (rdbCSharp.Checked ? "GroupDocs.Parser.Examples.CSharp" : "GroupDocs.Parser.Examples.VBasic"));
        ...
        bool CopyAndCreateProject()
        {
            ...
            //string sampleSourcePath = "GroupDocs.Parser";
            ...
            try
            {
                ...
                {
                    //For CSharp Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "GroupDocs.Parser.Examples.CSharp", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "GroupDocs.Parser.Examples.CSharp", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "GroupDocs.Parser.Examples.CSharp", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);

                    //For VBasic Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "GroupDocs.Parser.Examples.VBasic", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "GroupDocs.Parser.Examples.VBasic", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "GroupDocs.Parser.Examples.VBasic", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);
                }

                progressBar1.Value = 50;

                string[] projectFiles = Directory.GetFiles(Path.Combine(destinationPath, (rdbCSharp.Checked ? "GroupDocs.Parser.Examples.CSharp" : "GroupDocs.Parser.Examples.VBasic")), (rdbCSharp.Checked ? "*.csproj" : "*.vbproj"));
